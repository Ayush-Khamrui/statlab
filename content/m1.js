...
<div class="callout aiml" data-icon="🤖"><div class="callout-title">Where this sits in AI/ML</div><p>This <strong>is</strong> the EDA (exploratory data analysis) step every model starts with. You cannot model or predict data you haven't first understood. A skewed target, a hidden outlier, or a mis-typed feature silently wrecks downstream models — so a Tech Lead who reads data well prevents bugs that no amount of hyperparameter tuning will fix.</p></div>
...
<div class="callout interview" data-icon="💼"><div class="callout-title">Interview angle</div><p>"Why can't you take the mean of an ordinal feature like a 1–5 star rating?" — Because the gaps aren't guaranteed equal: the jump from 4→5 may not equal 1→2. The mean assumes <em>interval</em> spacing. Safe summary: the <strong>median</strong> or the <strong>mode</strong>. This single distinction explains why we one-hot encode nominal features but can label-encode ordinal ones.</p></div>
...
<div class="callout aiml" data-icon="🤖"><div class="callout-title">AI/ML connection</div><p>This robustness/efficiency trade-off recurs everywhere. <strong>L2 loss</strong> (MSE) is built on the mean → sensitive to outliers. <strong>L1 loss</strong> (MAE) is built on the median → robust. When a teammate asks "why is my regression chasing a few crazy points?", the answer is: squared error optimises toward a mean, and the mean follows the tail.</p></div>

<div class="callout example" data-icon="✏️"><div class="callout-title">Worked example</div><p>Morning routine (minutes) over 10 days: 39, 29, 43, 52, 39, 44, 40, 31, 44, 35. Sum = 396 → <b>mean = 39.6</b>. Sorted, the two middle values are 39 and 40 → <b>median = 39.5</b>. They nearly coincide ⟹ the data is roughly symmetric with no extreme outlier — a conclusion reached just by comparing two numbers.</p></div>

<div class="callout pitfall" data-icon="⚠️"><div class="callout-title">Pitfall</div><p>For <em>categorical</em> feedback ("excellent / good / fair / poor"), the mean is meaningless — you can't average labels. Use the <b>mode</b> (most frequent category). And a handy sanity rule for mildly skewed data: $\\text{Mode} \\approx 3\\,\\text{Median} - 2\\,\\text{Mean}$.</p></div>
...
`
    },
    {
      ...
      html: `
<p class="lead">The centre is only half the story. Two datasets can share the same mean yet feel completely different — one a sniper's tight cluster, the other a sprayer's chaos. <strong>Spread</strong> is what separates them, and it's often what we care about most: consistency, risk, reliability.</p>
...
<div class="callout aiml" data-icon="🤖"><div class="callout-title">AI/ML connection</div><p>The 1.5×IQR rule is the standard first pass for <strong>data cleaning</strong> before modelling, and it's literally what <code>matplotlib</code>/<code>seaborn</code> draw as the dots beyond box-plot whiskers. SD also defines <strong>standardisation</strong> ($z = (x-\\mu)/\\sigma$) — the scaling that lets gradient descent converge and that distance-based models (KNN, K-means, SVM) require.</p></div>
...
<div class="callout aiml" data-icon="🤖"><div class="callout-title">AI/ML connection · the log transform</div><p>Right-skewed features (revenue, counts, durations) often break linear models and inflate error on the tail. Applying $\\log(1+x)$ pulls the tail in, making the feature roughly symmetric and stabilising variance. This is one of the highest-ROI feature-engineering moves — and explains why metrics like RMSLE exist. <strong>Kurtosis</strong> (tail-heaviness) is the related idea behind "fat tails" in risk and anomaly detection.</p></div>
...
<div class="callout interview" data-icon="💼"><div class="callout-title">Three lines a Tech Lead should nail</div>
<ul>
