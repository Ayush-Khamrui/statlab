  ...
  sections: [
    {
      ...
      html: `
<p class="lead">Regression answers "given $x$, what's my best guess for $y$?" It is the workhorse of prediction and the foundation under most of supervised ML.</p>
...
<div class="callout pitfall" data-icon="⚠️"><div class="callout-title">Assumptions & their ML cousins</div><p>OLS assumes <strong>linearity</strong>, <strong>independent</strong> errors, <strong>constant variance</strong> (homoscedasticity), and roughly <strong>Normal</strong> residuals. Violations: a curved residual plot ⟹ add features/transform; fanning residuals ⟹ heteroscedasticity; <strong>multicollinearity</strong> (correlated predictors) makes coefficients unstable, which is the reason for ridge regression.</p></div>
...
<div class="callout aiml" data-icon="🤖"><div class="callout-title">Connection · this is already a neural net</div><p>Linear regression <em>is</em> a one-layer neural net with identity activation and MSE loss. Gradient descent here is the exact algorithm training every deep network. <strong>Regularisation</strong> controls the bias–variance trade-off and prevents overfitting: Ridge (L2, $+\\lambda\\sum\\beta_j^2$) shrinks all weights, while Lasso (L1, $+\\lambda\\sum|\\beta_j|$) drives weak features to zero. The learning-rate lesson from the demo (too big → diverge, too small → crawl) is the single most important practical hyperparameter in deep learning.</p></div>
...
<div class="callout definition" data-icon="📐"><div class="callout-title">The three components</div><p>Most series decompose into <b>Trend</b> (long-run direction) + <b>Seasonality</b> (repeating cycles such as daily, weekly, yearly) + <b>Noise</b> (irregular residual). Decomposition can be additive ($y=T+S+N$) or multiplicative ($y=T\\times S\\times N$).</p></div>

<div class="viz" data-viz="timeSeries"></div>

<div class="callout intuition" data-icon="🧠"><div class="callout-title">Intuition · smoothing reveals the signal</div><p>A <strong>moving average</strong> averages a sliding window to cancel noise and expose the trend, so a wider window is smoother but laggier. This is the seed idea of forecasting: separate the predictable signal (trend + seasonality) from the unpredictable noise, then project the signal forward.</p></div>

<h3>Stationarity: the key prerequisite</h3>
<p>A series is <strong>stationary</strong> if its statistical properties (mean, variance, autocorrelation) don't change over time. Most forecasting models (ARMA) assume it. Tame a non-stationary series by <strong>differencing</strong> ($y_t - y_{t-1}$ removes a trend) and seasonal differencing, which is the "I" (Integrated) in ARIMA.</p>
...
<div class="callout pitfall" data-icon="⚠️"><div class="callout-title">Pitfall · never shuffle a time series</div><p>You must split <strong>chronologically</strong>, training on the past and testing on the future. Random train/test splits leak the future into training and give fantasy accuracy. Evaluate with <strong>rolling/expanding-window backtesting</strong>, and watch for <strong>data leakage</strong> from look-ahead features.</p></div>
...
`,
  quiz: [
    ...
    { q: 'A forecasting model must, at minimum, beat which baseline to be worth deploying?', opts: ['A deep neural network', 'The naïve forecast ("tomorrow = today") and seasonal-naïve', 'A random number generator', 'The training mean'], answer: 1, explain: 'Naïve and seasonal-naïve are the standard sanity baselines. A complex model that can’t beat "tomorrow = today" or "same hour last week" adds cost without value, a check always worth making.' }
  ]
});
