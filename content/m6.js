...
<div class="callout intuition" data-icon="🧠"><div class="callout-title">Intuition · heights in a mixed crowd</div><p>Plot the heights of a room mixing adults and children and you see <em>two</em> humps, not one bell. Neither a single mean nor a single SD describes it well. A GMM models it as "45% drawn from a child-Gaussian + 55% from an adult-Gaussian." The hidden "which group?" label is a <strong>latent variable</strong>.</p></div>
...
<p>$\\pi_k$ = <b>mixing weight</b> (size of cluster $k$), $\\mu_k,\\Sigma_k$ = its mean and covariance. To <em>generate</em> a point: first pick a cluster with prob $\\pi_k$ (a categorical draw, see Module 3), then draw from that Gaussian.</p></div>
...
<p>A point between two clusters might be 60% cluster A, 40% cluster B, which is richer than a hard label.</p>
...
<div class="card"><h4>Responsibility</h4><p>$\\gamma_k(x)=\\dfrac{\\pi_k\\mathcal N(x\\mid\\mu_k,\\Sigma_k)}{\\sum_j\\pi_j\\mathcal N(x\\mid\\mu_j,\\Sigma_j)}$, the Bayes posterior over clusters (soft label).</p></div>
...
`,
  quiz: [
    { q: 'What fundamental problem does EM solve for a GMM?', opts: ['Choosing the number of clusters K', 'The chicken-and-egg of needing labels to fit Gaussians and Gaussians to assign labels', 'Computing the mean of one Gaussian', 'Removing outliers'], answer: 1, explain: 'If we knew each point’s cluster, fitting Gaussians is easy; if we knew the Gaussians, assigning points is easy. We know neither. EM alternates E (assign softly) and M (refit) to break the deadlock. It does NOT choose K; that’s the job of BIC/AIC.' },
